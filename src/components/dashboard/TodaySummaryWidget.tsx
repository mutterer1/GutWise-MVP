...
export default function TodaySummaryWidget({
  ...
  const getSnapshotSupportLine = () => {
    ...
    if (loggedCount === totalDomains) return `All ${totalDomains} core inputs captured`;
    if (loggedCount === 1) return 'Each entry sharpens today\'s picture';
    const remaining = totalDomains - loggedCount;
    if (remaining === 1) return 'One more input completes the picture';
    return `${remaining} inputs left to complete today`;
  };

  const getGuidanceStrip = (): { message: string; ctaText: string | null; ctaPath: string | null } => {
    if (loggedCount === totalDomains) {
      return { message: 'All core inputs are captured for today. Your picture is complete.', ctaText: null, ctaPath: null };
    }
    if (loggedCount === 0) {
      return { message: 'Log your first entry to begin today\'s snapshot.', ctaText: 'Start logging', ctaPath: '/bm-log' };
    }
    if (!domains[2].logged) {
      return { message: 'Hydration is the clearest missing input today.', ctaText: 'Log hydration', ctaPath: '/hydration-log' };
    }
    if (!domains[1].logged) {
  ...
  return (
    <section className="card-enter clinical-card clinical-priority-card p-5 sm:p-6 lg:p-7">
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
        <div className="flex min-w-0 flex-col justify-between gap-6">
          <div className="space-y-4">
            ...
            <div>
              <p className="data-kicker">Today&apos;s snapshot</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-[var(--color-text-primary)]">
                {getStatusHeadline()}
              </h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--color-text-secondary)]">
                {getSnapshotSupportLine()}. GutWise gets more useful when core entries overlap in
                the same day.
              </p>
            </div>
          </div>
        ...
        <aside className="flex flex-col items-center justify-center rounded-[28px] border border-[rgba(197,168,255,0.16)] bg-[rgba(7,10,24,0.34)] p-5">
          <p className="data-kicker mb-4">Core coverage</p>
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="max-w-[180px] text-center text-sm leading-6 text-[var(--color-text-secondary)]">
              {loggedCount} of {totalDomains} core inputs captured.
            </p>
